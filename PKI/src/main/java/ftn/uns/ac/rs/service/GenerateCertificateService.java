...
public class GenerateCertificateService {
	...
	public boolean createCertificate(CertificateDTO certificate, String trustStoreName) throws CertIOException {
		...
		if (certificate.getWho() == null) {
			...
			issuerData = ksr.readIssuerFromStore("./keystore/admin/" + k1.getKeystoreName(),
					certificate.getWho().getAlias(), k1.getPassword().toCharArray(),
					k1.getPrivateKeyPassword().toCharArray());
			certSign = (X509Certificate) ksr.readCertificate("./keystore/admin/" + k1.getKeystoreName(),
					k1.getPassword(), certificate.getWho().getAlias());
		}
		...
		if (temp == 1) {
			wr.write(cm.getId().toString(), issuerData.getPrivateKey(), certificate.getPrivatePassword().toCharArray(),
					cert);
		} else {
			wr.write(cm.getId().toString(), issuerData.getPrivateKey(), certificate.getPrivatePassword().toCharArray(),
					cert, certSign, certificate.getWho().getAlias());

		}
		wr.saveKeyStore("./keystore/admin/" + certificate.getKeystore(), certificate.getPassword().toCharArray());
		logger.info(USER, "Kreiran sertifikat");
		return true;
	}
	...
	public boolean createKeystore(KeystoreDTO keystoreDTO) {
		...
		if (keystoreDTO.getRole().equals("Admin store")) {
			wr.saveKeyStore("./keystore/admin/" + keystoreDTO.getKeystoreName(),
					keystoreDTO.getPassword().toCharArray());
		} else if (keystoreDTO.getRole().equals("Transfer store")) {
			wr.saveKeyStore("./keystore/distribution/transfer/" + keystoreDTO.getKeystoreName(),
					keystoreDTO.getPassword().toCharArray());
		} else {
			wr.saveKeyStore("./keystore/distribution/trusted/" + keystoreDTO.getKeystoreName(),
					keystoreDTO.getPassword().toCharArray());
		}
		return true;
	}
	...
	public Boolean addToTransfer(String alisa, String keystore) {
		...
		Certificate cert1 = k.readCertificate("keystore/admin/" + keyStoreADmin.getKeystoreName(), k1.getPassword(),
				alisa);
		IssuerData isuer = k.readIssuerFromStore("keystore/admin/" + keyStoreADmin.getKeystoreName(), alisa,
				keyStoreADmin.getPassword().toCharArray(), keyStoreADmin.getPrivateKeyPassword().toCharArray());

		KeyStoreWriter ksw = new KeyStoreWriter();
		ksw.loadKeyStore("./keystore/distribution/transfer/" + keyStoreTransfer.getKeystoreName(),
				keyStoreTransfer.getPassword().toCharArray());
		ksw.write(alisa, isuer.getPrivateKey(), keyStoreTransfer.getPrivateKeyPassword().toCharArray(), cert1);
		ksw.saveKeyStore("./keystore/distribution/transfer/" + keyStoreTransfer.getKeystoreName(),
				keyStoreTransfer.getPassword().toCharArray());
		logger.info(USER, "Sertifikat " + alisa + " dodat u keystore");
		return true;
	}
		...
		for (int i = 0; i < keystoreDTO.size(); i++) {
			...
			if (aliases != null) {
				while (aliases.hasMoreElements()) {
					...
					if (temp == 1)
						certificates.add(kr.readCertificate("keystore/admin/" + keystoreDTO.get(i).getKeystoreName(),
								keystoreDTO.get(i).getPassword(), s));
				}
			}
		}
	...
	public List<SignedSertificateDTO> getAllCertificates() {
		...
		for (CertificateM temp : k) {
			if (temp.getCa() == true) {
				...
				X509Certificate k2 = (X509Certificate) kr.readCertificate("./keystore/admin/" + k1.getKeystoreName(),
						k1.getPassword(), temp.getAlias());
